...
class Board:
    ...
    def print(self):
        ...
        for key in self.dict.keys():
            if self.dict[key].team == WHITE:
                temp.append([self.dict[key].count, key[0], key[1]])
        print_template['white'] = temp

        alt_temp = []
        for key in self.dict.keys():
            if self.dict[key].team == BLACK:
                temp.append([self.dict[key].count, key[0], key[1]])
    ...
    def move_white_piece(self, current_position, future_position, n, remainder):
        ...
        """
        remainder = self.dict[current_position].count - n
        future_stack_count = 0

        if future_position in self.dict and self.dict[future_position].team == WHITE:
            future_stack_count = self.dict[future_position].count
    ...
    def move_black_piece(self, current_position, future_position, n, remainder):
        ...
        if future_position in self.dict and self.dict[future_position].team == BLACK:
            future_stack_count = self.dict[future_position].count
